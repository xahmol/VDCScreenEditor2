...
class ViceMonitor:
    ...
    def wait_for_break_interactive(self, port: int = DEFAULT_PORT,
                                    timeout: float = 120.0,
                                    poll_interval: float = 1.0) -> int:
        """
        Disconnect from VICE so WSL2 X11 keyboard events reach VICE normally,
        then poll by briefly reconnecting every poll_interval seconds to check
        whether a breakpoint has fired.

        On WSL2/WSLg, keeping an active monitor socket open (especially while
        blocking in recv()) interferes with VICE's X11 keyboard delivery.
        Disconnecting releases that hold so physical key presses work again.

        With -keepmonopen, when a breakpoint fires VICE stays halted waiting
        for a client.  Python reconnects, reads the PC from the register dump,
        and if PC == $5577 (the overlay6 NOP break) returns immediately.
        Otherwise sends 'g' to resume, disconnects, and retries.

        Returns the PC address at the break (stays connected for caller to
        read memory / cont()).  Raises ViceMonitorError on timeout.
        """
        self.disconnect()
        ...
        while time.monotonic() < deadline:
            ...
            try:
                ...
                # Send 'r' to get a register dump (briefly halts CPU).
                result = self._recv_until_prompt(timeout=3.0, prime_with="r\n")

                # If a break notification is embedded in the response, we are
                # already at the break.
                m = self._BREAK_RE.search(result)
                if m:
                    return int(m.group(2), 16)

                # Parse PC from the VICE register dump (.;XXXX format).
                pc_m = re.search(r'\.\;([0-9a-fA-F]{4})', result)
                if pc_m:
                    pc = int(pc_m.group(1), 16)
                    if pc == 0x5577:   # the overlay6 NOP breakpoint
                        return pc

                # Not at the target break yet — resume and disconnect.
                self.cont()
                self.disconnect()

            except ViceMonitorError:
                # Timeout reading from socket — close and retry.
                try:
                    self._sock.close()
                except Exception:
                    pass
        ...
        raise ViceMonitorError(f"No breakpoint fired within {timeout}s")

    def keybuf_text(self, text: str, delay: float = 0.08):
        """
        Inject printable ASCII/PETSCII text character by character.

        Use for typing filenames in textInput dialogs.  Each char is injected
        individually (so the program can process each before the next arrives)
        with `delay` seconds between them.
        """
        for ch in text:
            self.keybuf(ch)
            time.sleep(delay)
